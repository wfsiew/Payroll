module EmployeeQualificationHelper
  ...
  def self.is_empty_params?(params)
    ...
    if q[:level] == '0' && q[:institute].blank? && q[:major].blank? && q[:year].blank? && q[:gpa].blank? &&
      q[:start_date].blank? && q[:end_date].blank?
      return true
    end
    false
  end
  ...
  def self.employee_qualification_obj(o, params)
    ...
    start_date = Date.strptime(_start_date, ApplicationHelper.date_fmt) if _start_date.present?
    end_date = Date.strptime(_end_date, ApplicationHelper.date_fmt) if _end_date.present?
    
    EmployeeQualification.new(:id => o.id, :level => q[:level], :institute => q[:institute], :major => q[:major], 
